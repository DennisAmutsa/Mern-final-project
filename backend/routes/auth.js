...
// Get all doctors (accessible by nurses and admins)
router.get('/doctors', authenticateToken, requireRole(['nurse', 'admin', 'doctor']), async (req, res) => {
  try {
...
router.post('/medications/:medicationId/administer', authenticateToken, requireRole(['nurse', 'admin']), [
  ...
  try {
    ...
    console.log('Medication administration request:', {
      medicationId,
      administeredAt,
      notes,
      status,
    ...
    const patient = await User.findOne({
      'currentMedications._id': new mongoose.Types.ObjectId(medicationId)
    });

    console.log('Found patient:', patient ? { id: patient._id, name: `${patient.firstName} ${patient.lastName}` } : 'Not found');
    ...
    // Update the specific medication's last administered info
